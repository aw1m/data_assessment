...
def ingest_questionnaire_attempt(cursor, sqliteConnection):
    ...
    for questionnaire_attempt in questionnaire_attempt_json:
        ...
        user_uuid, activity_uuid, questionnaire_uuid, attempt_number,submitted_at, created_at, updated_at, deleted_at,\
        context_uuid, question_uuid, confidence, type, single_answer_choice_uuid = questionnaire_attempt.values()
        cursor.execute("""
            SELECT *
         ...
         """, [user_uuid, activity_uuid, questionnaire_uuid, attempt_number,
               created_at, context_uuid,question_uuid, single_answer_choice_uuid])
        ...
        if records == []:
            try:
                ...
                print("records == []")
                print(e)
                print(user_uuid, activity_uuid, questionnaire_uuid, context_uuid, question_uuid,
                      single_answer_choice_uuid, attempt_number, confidence, type, submitted_at, created_at, updated_at,
                      deleted_at)
                print(records)

                not_processed.append(questionnaire_attempt)
        else:
            db_updated_at = records[0][11]
            if db_updated_at is None and updated_at is not None:
                try:
                    ...
                    print("db_updated_at is None and updated_at is not None")
                    print(e)
                ...
                try:
                    ...
                    print("db_updated_at is not None and db_updated_at < updated_at")
                    print(e)
                    print(user_uuid ,activity_uuid ,questionnaire_uuid ,context_uuid ,question_uuid ,
                          single_answer_choice_uuid ,attempt_number ,confidence ,type ,submitted_at  ,created_at ,updated_at ,deleted_at)
                    print(records)
                    not_processed.append(questionnaire_attempt)
            else:
                not_processed.append(questionnaire_attempt)
